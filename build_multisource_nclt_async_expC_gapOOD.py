...
CONFIG = {
    ...
    # 行格式近似：time, mag_x, mag_y, mag_z, acc_x, acc_y, acc_z, gyro_x, gyro_y, gyro_z
    "imu_acc_cols": [4, 5, 6],
    "imu_gyro_cols": [7, 8, 9],
    "imu_use_real_values": True,
    "imu_auto_axis_calib": True,

    # 传感器目标保留频率（用真实时间戳，再做稀疏化）
    ...
    # 观测退化协议
    # 注意：当前版本只把 IMU 切成真实数值；odom / gps 仍是 latent truth + degradation。
    "protocols": {
        "odom": {
            "bernoulli_keep": 0.97,
    ...
    # Exp-C gap protocol:
    # train: seen gaps only; valid: seen + mild extrapolation; test: longer / OOD gaps.
    "split_gps_gap_lengths_sec": {
        "train": [1.0, 2.0, 5.0],
        "valid": [1.0, 2.0, 5.0, 8.0],
        "test": [5.0, 8.0, 10.0],
    },
...
def _resolve_config(cfg: Dict) -> Dict:
    ...
    if out["out_root"] is None:
        out["out_root"] = str(script_dir / "Data_NCLT_MultiSourceAsync_CTRA6_RealIMU_ExpC_GapOOD")
    return out
...
def _make_protocols_for_split(cfg: Dict, split: str) -> Dict:
    """Return a deep-copied degradation protocol with split-specific GPS gap lengths."""
    split = str(split).lower()
    protocols = copy.deepcopy(cfg["protocols"])
    gap_cfg = cfg.get("split_gps_gap_lengths_sec", {})
    if split not in gap_cfg:
        raise KeyError(f"Missing split_gps_gap_lengths_sec for split={split}")
    protocols["gps"]["gap_lengths_sec"] = [float(x) for x in gap_cfg[split]]
    return protocols


def _time_slice_day_by_seconds(day: DayData, start_s: float, end_s: Optional[float] = None) -> DayData:
    """Slice by local seconds, then reuse _time_slice_day to reset time and dt_last consistently."""
    t = day.t_event.astype(np.float64)
    if end_s is None:
        keep = t >= float(start_s)
    else:
        keep = (t >= float(start_s)) & (t < float(end_s))
    idx = np.where(keep)[0]
    if idx.size <= 10:
        raise ValueError(
            f"Time slice too short: start_s={start_s}, end_s={end_s}, kept_events={idx.size}"
        )
    return _time_slice_day(day, int(idx[0]), int(idx[-1]) + 1)


def _print_split_protocols(cfg: Dict) -> None:
    gap_cfg = cfg.get("split_gps_gap_lengths_sec", {})
    print("Exp-C GPS gap protocol:")
    print(f"  train: {gap_cfg.get('train')} sec")
    print(f"  valid: {gap_cfg.get('valid')} sec")
    print(f"  test : {gap_cfg.get('test')} sec")
...
def main():
    ...
    print("=" * 100)
    print("Building NCLT multi-source async dataset (Exp-C gap-OOD protocol, REAL IMU)")
    ...
    _print_split_protocols(cfg)
    print("=" * 100)

    train_protocols = _make_protocols_for_split(cfg, "train")
    valid_protocols = _make_protocols_for_split(cfg, "valid")
    test_protocols = _make_protocols_for_split(cfg, "test")

    # Train and valid follow the original Exp-B split policy on the same NCLT day,
    # but their GPS degradation protocol is generated separately so that validation
    # includes the additional 8 s extrapolation gap.
    ...
    valid_day_full = preprocess_one_day_multisource(
        gt_csv=train_files["gt"],
        odom_csv=train_files["odom"],
        gps_csv=train_files["gps"],
        imu_csv=train_files["imu"],
    ...
    split_time_sec = float(train_day_full.t_event[split_idx])

    train_day = _time_slice_day_by_seconds(train_day_full, 0.0, split_time_sec)
    valid_day = _time_slice_day_by_seconds(valid_day_full, split_time_sec, None)

    print("=" * 100)
    print("Resolved split protocol:")
    print(f"train gps gaps: {train_protocols['gps']['gap_lengths_sec']} | windows {cfg['train_duration_sec']}s/{cfg['train_stride_sec']}s")
    print(f"valid gps gaps: {valid_protocols['gps']['gap_lengths_sec']} | windows {cfg['valid_duration_sec']}s/{cfg['valid_stride_sec']}s")
    print(f"test  gps gaps: {test_protocols['gps']['gap_lengths_sec']} | windows {cfg['test_duration_sec']}s/{cfg['test_stride_sec']}s")
    print(f"train/valid split_time_sec: {split_time_sec:.3f}")
    print("=" * 100)
    ...
    save_cfg = dict(cfg)
    save_cfg["state_mode"] = "ctra6_latent_async_multisource_realimu_expC_gapOOD"
    ...
    save_cfg["exp_name"] = "Exp-C gap-OOD: train GPS gaps [1,2,5], valid [1,2,5,8], test [5,8,10]"
    save_cfg["resolved_protocols"] = {
        "train": train_protocols,
        "valid": valid_protocols,
        "test": test_protocols,
    }
    save_cfg["train_valid_split_time_sec"] = split_time_sec
