...
def compute_train_stats(out_root: Path) -> Dict:
    ...
    stats = {
        "state_mode": "ctra6_latent_async_multisource_realimu_expA_easy_matched",
...
CONFIG = {
    ...
    # 观测退化协议 —— Exp-A Easy matched
    # 目标：轻缺测、无长 gap、轻 jitter、噪声正常；不加入 reliability shift。
    "protocols": {
        "odom": {
    ...
    # 切窗协议：保留当前主线 train/valid/test 时长，保证和 Exp-B 可对比；
    # “matched” 体现在 train/test 都采用 easy protocol，而不是改时间长度。
    "train_split": 0.875,
...
def _resolve_config(cfg: Dict) -> Dict:
    ...
    if out["out_root"] is None:
        out["out_root"] = str(script_dir / "Data_NCLT_MultiSourceAsync_CTRA6_RealIMU_ExpA_EasyMatched")
    return out
...
def main():
    ...
    print("=" * 100)
    print("Building NCLT multi-source async dataset (Exp-A Easy matched, REAL IMU)")
    ...
    save_cfg = dict(cfg)
    save_cfg["state_mode"] = "ctra6_latent_async_multisource_realimu_expA_easy_matched"
